...
def _run_loco(loco_client: Path, network_interface: str, *args: str) -> None:
    ...
    env = os.environ.copy()
    # ROS 2 Jazzy prepends its CycloneDDS libraries to LD_LIBRARY_PATH. The
    # Unitree SDK binary was built against the SDK-installed CycloneDDS in
    # /usr/local/lib; mixing the two can abort inside libddsc cleanup.
...
def main() -> int:
    ...
    if all(key in spec for key in ("vx", "vy", "yaw", "duration")):
        ...
        print(
            "robot_skill planned "
            f"command={command} execute={execute} "
            f"vx={vx:.3f} vy={vy:.3f} yaw={yaw:.3f} duration={duration:.3f}",
            flush=True,
        )
        if not execute:
            return 0
